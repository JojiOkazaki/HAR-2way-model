...
def write_video_sample(video_dir, json_dir, out_img_dir, out_skel_dir, label, label_id, video_filename, T, N, K, W, H):
    # 出力先ディレクトリの取得
    ...
    # 既にptファイルがある場合は処理しない
    if os.path.exists(out_img_path) and os.path.exists(out_skel_path):
        return
    ...
    # 処理
    frames, img_data, skel_data, score_data = generate_video_sample(video_path, json_data, T, N, K, W, H)

    # ptファイルを作成
    torch.save(
        {"images": img_data, "frames": frames, "label": label_id},
        out_img_path,
    )
    torch.save(
        {"skeletons": skel_data, "scores": score_data, "frames": frames, "label": label_id},
        out_skel_path,
    )

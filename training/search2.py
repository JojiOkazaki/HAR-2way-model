...
---------------------
1) Run inference for (all) preprocessed video samples (.pt) and build an embedding index.
2) Encode a text query with SentenceTransformer and retrieve Top-N most similar videos.

Index granularity
-----------------
- This version stores **one embedding per person** (not averaged).
- Default search output is still **Top-N videos** by taking the *best* (max) similarity among persons
  within the same .pt sample.
- Use --by_person to retrieve Top-N person instances instead.
...
# One-shot query (auto-build index if missing)
...
def _torch_load_pt(path: Path) -> dict:
    try:
...
_label_ids_cache: Dict[str, List[int]] = {}

def get_label_id_from_pt(img_pt_dir: Path, relpath: str, person_id: int) -> int:
    """
    img_pt_dir / relpath の .pt を開き、label_ids[person_id] を返す。
    無い/範囲外は -1。
    """
    key = str(relpath)
    lids = _label_ids_cache.get(key)
    if lids is None:
        data = _torch_load_pt(img_pt_dir / relpath)
        raw = data.get("label_ids", None)

        if raw is None:
            lids = []
        elif isinstance(raw, torch.Tensor):
            lids = [int(x) for x in raw.detach().cpu().tolist()]
        else:
            lids = [int(x) for x in list(raw)]

        _label_ids_cache[key] = lids

    pid = int(person_id)
    return int(lids[pid]) if 0 <= pid < len(lids) else -1
...
class VideoEmbeddingIndex:
    ...
    - relpaths: length N (per-person row -> which .pt sample)
    - person_ids: length N (per-person row -> which person index inside that sample)
    """
    ...
    def __post_init__(self):
        ...
        n = int(self.embeddings.size(0))
        if len(self.relpaths) != n:
            raise ValueError("relpaths length mismatch")
        if len(self.person_ids) != n:
            raise ValueError("person_ids length mismatch")
...
def load_index(path: Path) -> VideoEmbeddingIndex:
    ...
    # backward compatibility: older index may not have person_ids
    if person_ids is None:
        person_ids_list = [0] * n
    elif isinstance(person_ids, torch.Tensor):
        person_ids_list = [int(x) for x in person_ids.detach().cpu().tolist()]
    else:
        person_ids_list = [int(x) for x in list(person_ids)]

    return VideoEmbeddingIndex(
...
def build_video_index(
    ...
    """
    Build a person-level embedding index.

    - One embedding per *valid person*.
    - A "valid person" is defined by min_valid_t frames having any visible keypoint (score>0).
    ...
    all_embs: List[torch.Tensor] = []  # list of (Nb,D) CPU tensors
    all_paths: List[str] = []
    all_pids: List[int] = []

    stats = {
        "videos_seen": 0,
        "videos_indexed": 0,  # number of dataset items having >=1 valid person
        "videos_skipped_no_valid_person": 0,
        "persons_total": 0,
        "persons_valid": 0,
        "persons_indexed": 0,  # equals persons_valid in this builder
    }

    for batch in tqdm(loader, desc="Build person embeddings"):
        ...
        video_has_valid = person_valid.any(dim=1)            # (B,)
        stats["videos_indexed"] += int(video_has_valid.sum().item())
        stats["videos_skipped_no_valid_person"] += int((~video_has_valid).sum().item())
        ...
        # map each person to its video id (0..B-1) and person id (0..P-1)
        video_ids_flat = torch.arange(B).repeat_interleave(P)  # (B*P,)
        person_ids_flat = torch.arange(P).repeat(B)            # (B*P,)

        video_ids = video_ids_flat[idx]                        # (N,)
        person_ids = person_ids_flat[idx]                      # (N,)
        ...
        stats["persons_indexed"] += int(idx.numel())
        ...
        emb = F.normalize(emb, dim=-1, eps=1e-6).detach()  # (N,D)
        all_embs.append(emb.cpu())

        vids = video_ids.tolist()
        pids = person_ids.tolist()
        all_paths.extend([relpaths[v] for v in vids])
        all_pids.extend([int(x) for x in pids])

        if max_videos is not None and stats["videos_seen"] >= int(max_videos):
            break

    if not all_embs:
        raise RuntimeError("No person embeddings were created (all samples might have no valid persons).")

    embs = torch.cat(all_embs, dim=0).contiguous().float()  # (N,D) on CPU
    ...
    index = VideoEmbeddingIndex(
        ...
        meta={
            ...
            "persons_total": int(stats["persons_total"]),
            "persons_valid": int(stats["persons_valid"]),
            "persons_indexed": int(stats["persons_indexed"]),
            "index_granularity": "person",
        },
    )
    return index, stats
...
def _check_dim(index: VideoEmbeddingIndex, query_emb: torch.Tensor) -> None:
    if query_emb.ndim != 1:
...
def search_topk_by_person(index: VideoEmbeddingIndex, query_emb: torch.Tensor, topk: int) -> List[Tuple[int, float, str, int]]:
    """
    Returns list of (rank, score, relpath, person_id)
    """
    _check_dim(index, query_emb)
    ...
    for r, (i, v) in enumerate(zip(idxs.tolist(), vals.tolist()), 1):
        out.append((r, float(v), index.relpaths[int(i)], int(index.person_ids[int(i)])))
    return out


def search_topk_by_video(index: VideoEmbeddingIndex, query_emb: torch.Tensor, topk: int) -> List[Tuple[int, float, str, int]]:
    """
    Person-level index -> video-level retrieval by max pooling per relpath.

    Returns list of (rank, best_score, relpath, best_person_id)
    """
    _check_dim(index, query_emb)
    ...
    # zip over Python lists for speed
    for rel, pid, s in zip(index.relpaths, index.person_ids, sims.tolist()):
        cur = best.get(rel)
        if (cur is None) or (s > cur[0]):
            best[rel] = (float(s), int(pid))

    items = sorted(best.items(), key=lambda kv: kv[1][0], reverse=True)
    k = min(int(topk), int(len(items)))
    out = []
    for r, (rel, (score, pid)) in enumerate(items[:k], 1):
        out.append((r, float(score), rel, int(pid)))
    return out
...
def main():
    ...
    parser.add_argument(
        "--by_person",
        action="store_true",
        help="Return Top-N person instances (default: group by video and return Top-N videos by max over persons).",
    )
    ...
    if args.index_path:
        ...
        index_path = (run_dir / "search" / f"person_index_{args.target}.pt").resolve()
    ...
    if args.build or (not index_path.exists()):
        ...
        print(
            "[Index] done. "
            f"videos_indexed={stats['videos_indexed']} "
            f"persons_indexed={stats['persons_indexed']} "
            f"skipped_videos={stats['videos_skipped_no_valid_person']}"
        )
    else:
        ...
        print(
            "[Index] loaded. "
            f"rows={index.embeddings.size(0)}, dim={index.embeddings.size(1)}, "
            f"target={index.meta.get('target')}, granularity={index.meta.get('index_granularity')}"
        )
    ...
    def run_query(q: str) -> List[Tuple[int, float, str, int]]:
        q_emb = encode_text(st_model, q)
        if args.by_person:
            return search_topk_by_person(index, q_emb, topk=args.topk)
        return search_topk_by_video(index, q_emb, topk=args.topk)
    ...
    if args.query is not None:
        ...
        results = run_query(q)
        print(f"\nQuery: {q}")
        for rank, score, relpath, pid in results:
            out_path = format_output_path(relpath, args.output_ext, args.basename)
            lid = get_label_id_from_pt(img_pt_dir, relpath, pid)
            print(f"{rank:2d}. {score:+.4f}\t{out_path}\tperson={pid}\tlabel_id={lid}")
        return
